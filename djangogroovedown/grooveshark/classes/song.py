...
class Song(object):
    ...
    def export(self):
        ...
        :class:`Song` object.
        """
        return {'id' : self.id, 'name' : self.name.replace('\"', '\''), 'artist' :self._artist_name.replace('\"', '\''), 'artist_id' : self._artist_id,
                'album' : self._album_name.replace('\"', '\''), 'album_id' : self._album_id, 'track' : self.track,
    ...
    def format(self, pattern):
        ...
        %A: album title
        """
        pattern = pattern.replace('%i', str(self.id))
    ...
    def download(self, directory='~/Music', song_name='%i %a - %s - %A'):
        """
